...
// Secrets required (hs secret add):
//   SUPABASE_URL
...
exports.main = async (context = {}, sendResponse) => {
  try {
    ...
    // 1. Look up mediator by HubSpot mediator object record ID
    ...
    if (!mediators.length) {
      return sendResponse({
        status:  'ERROR',
        message: `No portal account linked to mediator record ${hs_object_id}. Set hubspot_mediator_object_id in Supabase.`,
      })
    }
    ...
    const slots     = Array.isArray(allSlots)
      ? allSlots.filter(s => s.date <= date_to)
      : []
