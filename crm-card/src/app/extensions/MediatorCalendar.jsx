...
// ─── Series resolution (mirrors PWA logic) ──────────────────────
function matchesFrequency(series, date) {
  ...
  if (series.frequency === 'monthly') {
    const start         = new Date(series.start_date)
    const weekOfStart   = Math.ceil(start.getDate() / 7)
    const weekOfCurrent = Math.ceil(date.getDate() / 7)
    return weekOfStart === weekOfCurrent
  }
  return true
}
...
function resolveSlot(date, period, slots, series) {
  ...
  const dayOfWeek = rawDay === 0 ? 6 : rawDay - 1 // 0=Mon
  ...
  if (match) return { status: match.status, notes: match.notes, source: 'series' }
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (resp.status === 'SUCCESS') {
        ...
        setError(resp.message || 'Failed to load availability data.')
      }
    } catch (e) {
      setError(e.message)
    }
  ...
  if (error) {
    return (
      <Flex direction="column" gap="small">
        <Alert title="Could not load availability" variant="error">
          {error}
        </Alert>
  ...
  return (
    <Flex direction="column" gap="small">
      ...
      <Flex align="center" gap="small">
        ...
        <Box flex={1}>
          <Text format={{ bold: true }} align="center">
      ...
      <Flex direction="column" gap="extra-small">
        {days.map(day => {
          ...
          const allEmpty = amSlot.status === 'not_set' && pmSlot.status === 'not_set'

          return (
            <Box
              key={dateStr}
              padding="small"
              border={isToday ? 'medium' : 'none'}
              borderColor={isToday ? 'active' : undefined}
            >
              <Flex direction="column" gap="extra-small">
                ...
                {allEmpty ? (
                  ...
                  <Flex direction="row" gap="small" wrap="wrap">
                    <SlotBadge slot={amSlot} label="AM" />
                    <SlotBadge slot={pmSlot} label="PM" />
                  </Flex>
                )}
              </Flex>
            </Box>
      ...
      <Flex direction="row" gap="extra-small" wrap="wrap">
        {Object.entries(STATUS).filter(([k]) => k !== 'not_set').map(([key, cfg]) => (
          <Tag key={key} variant={cfg.variant}>{cfg.label}</Tag>
        ))}
      </Flex>
      ...
      {/* Actions */}
      <Flex justify="space-between" align="center">
        <Button onClick={fetchData} variant="secondary" size="xs">
          Refresh
        </Button>
        <Button
        ...
        >
          {requested ? 'Request sent ✓' : requesting ? 'Sending…' : 'Request availability update'}
        </Button>
      </Flex>
